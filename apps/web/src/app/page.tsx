...
function cToF(c: number | undefined) {
  if (typeof c !== "number") return c as any;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!active) return;
        setSuggestions([]);
        setError(e.message || "Suggestion error");
      }
    })();
    return () => {
      active = false;
        ...
        navigator.geolocation.getCurrentPosition(
          async (pos) => {
            ...
            try {
              ...
              // Use name if available, else coords, else last, else fallback
              const label =
                rj?.results?.[0]?.name ||
                last ||
